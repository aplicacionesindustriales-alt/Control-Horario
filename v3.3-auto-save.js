...
let busy=false;
async function cloudCatalog(ws){
  const x=await C.client.functions.invoke('worker-api',{body:{action:'data',token:ws.token}});
  if(x.error)throw x.error;if(x.data?.error)throw new Error(x.data.error);
  return x.data||{};
}
function remapReport(r,d){
  ...
  const pByCode=new Map(projects.map(p=>[String(p.code||'').toLowerCase(),p]));
  const tById=new Map(tasks.map(t=>[t.id,t]));
  const tByName=new Map(tasks.map(t=>[String(t.name||'').toLowerCase(),t]));
  const lines=(r.lines||[]).map(l=>{
    ...
    return {...l,projectId:op?.id||l.projectId,taskId:ot?.id||l.taskId,hours:hours(l.start,l.end,l.break)};
  });
  return {...r,lines};
}
async function syncWorkerReports(){
 ...
 try{
  const rs=reports(),mp=maps(),d=await cloudCatalog(ws);
  for(let i=0;i<rs.length;i++){
   const r=rs[i];if(r.workerId!==ws.employee_id)continue;
   const mapped=remapReport(r,d);const cloudId=mp[r.id]||r.cloud_id||null;
   const report={...mapped,cloud_id:cloudId};
   const x=await C.client.functions.invoke('worker-api',{body:{action:'save',token:ws.token,report}});
   if(x.error)throw x.error;if(x.data?.error)throw new Error(x.data.error);
   if(x.data?.timesheet_id){mp[r.id]=x.data.timesheet_id;rs[i]={...mapped,cloud_id:x.data.timesheet_id}}
  }
  saveMaps(mp);saveReports(rs)
 }catch(e){
  console.error('Control Horario: error guardando en nube',e);
  const detail=e?.message||e?.context?.message||'Error desconocido';
  alert('El parte se ha guardado localmente, pero NO se pudo guardar en la nube.\n\nMotivo: '+detail+'\n\nComprueba la conexión e inténtalo de nuevo.')
 }finally{busy=false}
}
