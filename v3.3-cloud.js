...
async function signup(){try{msg('Creando cuenta…');const r=await C.auth.signUp(document.getElementById('cloudEmail').value.trim(),document.getElementById('cloudPass').value);if(r.error)throw r.error;msg(r.data.session?'Cuenta creada.':'Revisa el email para confirmar la cuenta y después entra.')}catch(e){msg(e.message||'Error')}}
...
async function cloudData(){const company=profile.company_id;const workerOnly=profile.role==='worker';
  const [e,p,t,pt,ts]=await Promise.all([
    ...
    workerOnly
      ? C.client.from('timesheets').select('*,timesheet_lines(*)').eq('company_id',company).eq('employee_id',profile.employee_id).order('work_date',{ascending:false})
...
async function boot(){try{await loadProfile();if(!profile){const r=await C.client.functions.invoke('bootstrap-admin',{body:{name:session.user.email?.split('@')[0]||'Administrador'}});if(!r.error){await loadProfile()}else{render();msg('Cuenta sin perfil. El administrador debe darte de alta.');return}}if(!profile){render();msg('Cuenta sin perfil.');return}if(!sessionStorage.getItem('ch_cloud_loaded')){await cloudData();sessionStorage.setItem('ch_cloud_loaded','1');location.reload();return}render()}catch(e){render();msg(e.message||'No se pudo cargar la nube')}}
